...
router.post("/", async (req, res) => {
	...
	if (body.success) {
		...
		createUser(username, password).then((added_user) => {
			if (added_user != undefined) {
				...
				res.status(409).send({ error: true, message: "Username already taken" })
			}

		}).catch((e: any) => {
			res.status(503).send({ error: true, message: "Bad interaction with DB: " + e.message })
		})
	}
	else {
		res.status(400).send({ error: true, message: body.error.issues[0].path + ' - ' + body.error.issues[0].message })
	}
});
